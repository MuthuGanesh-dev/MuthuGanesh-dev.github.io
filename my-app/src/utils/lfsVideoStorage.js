 ...
 * @param {string} videoFilename - The filename to delete (e.g., "my-video-123456.mp4")
 * @returns {Promise<{success: boolean, message: string}>}
 */
export async function deleteVideoFromLFS(videoFilename) {
  const token = import.meta.env.VITE_GITHUB_TOKEN;
  ...
  try {
    const filePath = `${VIDEOS_FOLDER}/${videoFilename}`;
    ...
    const deleteResponse = await fetch(fileUrl, {
      ...
      body: JSON.stringify({
        message: `Delete video: ${videoFilename}`,
        sha: fileData.sha,
        branch: GITHUB_BRANCH,
      }),
    });
    ...
    console.error("LFS video delete error:", error);
    return {
      success: false,
